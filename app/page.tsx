...
export default function Page() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      {budgetEditor && <div className="fixed inset-0 z-50 flex items-end justify-center bg-foreground/35 p-0 sm:items-center sm:p-4"><section role="dialog" aria-modal="true" aria-labelledby="budget-title" className="w-full rounded-t-3xl bg-card p-6 shadow-2xl sm:max-w-md sm:rounded-2xl"><div className="mb-6 flex items-start justify-between"><div><p className="font-mono text-xs uppercase tracking-widest text-primary">Límite mensual</p><h2 id="budget-title" className="mt-1 font-serif text-2xl font-semibold">Presupuesto {budgetEditor}</h2></div><button type="button" onClick={() => setBudgetEditor(null)} className="rounded-lg p-2 text-muted-foreground hover:bg-muted" aria-label="Cerrar"><X /></button></div><form onSubmit={saveBudget} className="flex flex-col gap-4"><label className="flex flex-col gap-2 text-sm font-medium">Presupuesto de gasto mensual objetivo<input autoFocus required type="number" min="0.01" step="0.01" value={budgetForm} onChange={(event) => setBudgetForm(event.target.value)} className="rounded-xl border border-input bg-background px-3 py-3 font-normal outline-none focus:ring-2 focus:ring-ring" /></label><div className="flex justify-end gap-2"><button type="button" onClick={() => setBudgetEditor(null)} className="rounded-xl border border-border px-4 py-2 text-sm font-medium hover:bg-muted">Cancelar</button><button type="submit" className="rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:opacity-90">Guardar límite</button></div></form></section></div>}
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
        ...
        <section className="mb-6 grid gap-4 md:grid-cols-[1.4fr_1fr_1fr]">
          <div className="rounded-2xl bg-primary p-6 text-primary-foreground"><div className="flex items-start justify-between gap-4"><div><p className="text-sm text-primary-foreground/70">Gasto total</p><p className="mt-2 font-serif text-4xl font-semibold">{money.format(total)}</p></div><CircleDollarSign className="size-7 shrink-0 opacity-70" /></div><div className="mt-8 flex items-center gap-2 text-sm text-primary-foreground/75"><TrendingUp className="size-4" /> Este mes · {filtered.length} movimientos</div></div>
          <div className="rounded-2xl border border-border bg-card p-6"><div className="flex items-center justify-between gap-3"><p className="text-sm text-muted-foreground">Profesional</p><div className="flex items-center gap-2"><span className="text-xs text-muted-foreground">{professionalBudget > 0 ? money.format(professionalBudget) : 'Sin límite'}</span><button type="button" onClick={() => openBudgetEditor('Profesional')} className="rounded-lg p-1.5 text-muted-foreground transition hover:bg-muted hover:text-foreground" aria-label="Editar presupuesto profesional" title="Editar presupuesto"><Pencil className="size-4" /></button><BriefcaseBusiness className="size-5 text-primary" /></div></div><p className="mt-4 font-serif text-3xl font-semibold">{money.format(professionalTotal)}</p><div className="mt-4 h-1.5 overflow-hidden rounded-full bg-muted"><div className={`h-full rounded-full ${professionalPercentage > 100 ? 'bg-red-500' : 'bg-primary'}`} style={{ width: `${Math.min(professionalPercentage, 100)}%` }} /></div><p className="mt-2 text-xs text-muted-foreground">{professionalBudget > 0 ? `${Math.round(professionalPercentage)}% del presupuesto` : 'Define un presupuesto mensual'}</p></div>
          <div className="rounded-2xl border border-border bg-card p-6"><div className="flex items-center justify-between gap-3"><p className="text-sm text-muted-foreground">Personal</p><div className="flex items-center gap-2"><span className="text-xs text-muted-foreground">{personalBudget > 0 ? money.format(personalBudget) : 'Sin límite'}</span><button type="button" onClick={() => openBudgetEditor('Personal')} className="rounded-lg p-1.5 text-muted-foreground transition hover:bg-muted hover:text-foreground" aria-label="Editar presupuesto personal" title="Editar presupuesto"><Pencil className="size-4" /></button><UserRound className="size-5 text-chart-3" /></div></div><p className="mt-4 font-serif text-3xl font-semibold">{money.format(personalTotal)}</p><div className="mt-4 h-1.5 overflow-hidden rounded-full bg-muted"><div className={`h-full rounded-full ${personalPercentage > 100 ? 'bg-red-500' : 'bg-chart-3'}`} style={{ width: `${Math.min(personalPercentage, 100)}%` }} /></div><p className="mt-2 text-xs text-muted-foreground">{personalBudget > 0 ? `${Math.round(personalPercentage)}% del presupuesto` : 'Define un presupuesto mensual'}</p></div>
        </section>

        <section className="mb-8 grid gap-6 lg:grid-cols-[0.9fr_1.6fr]">
          <div className="min-w-0 rounded-2xl border border-border bg-card p-4 sm:p-6"><div className="mb-5 flex items-start justify-between gap-3 sm:mb-6"><div className="min-w-0"><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground sm:text-xs sm:tracking-widest">Distribución</p><h3 className="mt-1 font-serif text-lg font-semibold sm:text-xl">Por categoría</h3></div><BarChart3 className="size-5 shrink-0 text-muted-foreground" /></div><div className="flex min-w-0 flex-col gap-4">{topCategories.length ? topCategories.map(([name, value], index) => <div key={name}><div className="mb-1.5 flex justify-between text-sm"><span className="flex items-center gap-2"><span className={`size-2.5 rounded-full ${['bg-primary', 'bg-chart-3', 'bg-chart-4', 'bg-chart-2'][index]}`} />{name}</span><span className="font-medium">{money.format(value)}</span></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className={`h-full rounded-full ${['bg-primary', 'bg-chart-3', 'bg-chart-4', 'bg-chart-2'][index]}`} style={{ width: `${Math.min(value / maxCategory * 100, 100)}%` }} /></div></div>) : <p className="text-sm text-muted-foreground">No hay datos para este filtro.</p>}</div></div>
